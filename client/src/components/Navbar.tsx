...
import {
  ...
  Megaphone, Activity, CheckSquare, Sparkles, Clock, Shield, Globe,
  Palette, DollarSign, Volume2, Image, Bot, Eye, Code, AlertTriangle, LineChart,
  ExternalLink, Menu, X, MapPin, FileCheck, Settings, Package, UserPlus
} from 'lucide-react';
...
function Navbar(_props: NavbarProps) {
  ...
  // Click outside to close dropdowns (not the mobile menu)
  useEffect(() => {
  ...
  const handleNavigation = useCallback((route: string, tabName: string) => {
    ...
    // setActiveTab handled by useEffect
  }, [navigate]);
  ...
  useEffect(() => {
    ...
    else if (path === '/assistants') setActiveTab('assistants');
                      ...
                      <div key={tab.id} className="relative shrink-0">
                        ...
                        >
                          ...
                          {tab.badge && renderBadge(
                            ...
                            tab.id === 'assistants' ? 'bg-emerald-500' :
                            tab.id === 'admin' ? 'bg-red-500' : 'bg-blue-500'
                          )}
                          ...
                          {isActive && <div className={`absolute inset-0 bg-gradient-to-r ${tab.color} rounded-full opacity-20 animate-pulse`} />}
                        </button>
                  ...
                  {dropdownMenus.map((menu) => (
                    <div key={menu.id} className="relative shrink-0">
                      ...
                      >
                        ...
                        {activeDropdown === menu.id && (
                          <div className={`absolute inset-0 bg-gradient-to-r ${menu.color} rounded-full opacity-20 animate-pulse`} />
                        )}
                      </button>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const routeMap: { [key: string]: string } = {
                                    ...
                                    'circle-prospecting': '/communication',
                                    'forms': '/forms',
                      ...
                      {/* Business Intelligence fullscreen */}
