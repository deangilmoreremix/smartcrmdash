...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      // Check if the click is inside a dropdown toggle button
      const isDropdownToggle = (e.target as HTMLElement).closest('[data-dropdown-toggle]');
  ...
  const handleAIToolClick = (toolName: string) => {
    ...
    if (toolRoutes[toolName]) {
      ...
      // For AI tools, use the openAITool function from NavigationContext
      openAITool(toolName);
    }
    setActiveDropdown(null);
    setIsMobileMenuOpen(false);
  ...
  // Define dropdown menu configurations after the tools arrays
