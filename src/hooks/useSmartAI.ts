...
export const useTaskOptimization = () => {
  ...
  // Mock recommendations data (in a real app, this would come from analytics or be computed)
  ...
  const performance: TaskOptimizationMetrics = {
    ...
    avgResponseTime: 872, // in milliseconds
    modelPerformance: [
      {
        model: 'gemini-2.5-flash',
        successRate: 0.97,
        avgTime: 1240, // in milliseconds
        avgCost: 0.0045,
      ...
      },
      {
        ...
        avgTime: 820,
        avgCost: 0.0018,
        taskTypes: ['quick_responses', 'basic_tasks']
      }
    ]
  };
  ...
  const getInsights = useCallback(async (data: any, customerId?: string) => {
    ...
    try {
      const insights = await enhancedGeminiService.generateInsights(data, customerId);
      return insights;
    } catch (err) {
...
export const useSmartAI = () => {
  ...
  return { getAIModels, getUsageStats, isLoading, error };
};